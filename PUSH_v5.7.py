...
# -*- coding: utf-8 -*-
"""
PUSH_v5.7.py — δημοσίευση της ΑΣΠΙΔΑΣ v5.7 (build VV, 574 έλεγχοι) στο GitHub.
...
ΤΙ ΚΑΝΕΙ (με τη σειρά)
  ...
  4. δημιουργία release v5.7 με τις σημειώσεις από docs/RELEASE_NOTES_v5.7.md.
...
ΧΡΗΣΗ
  GH_TOKEN=ghp_... python3 publish/PUSH_v5.6.py
...
if not TOKEN:
    print('✗ Λείπει το token.  Χρήση:  GH_TOKEN=... python3 publish/PUSH_v5.7.py'); sys.exit(2)
...
print('══ 2. TAG v5.7 ══')
git('push', '--force', 'https://github.com/%s.git' % REPO, 'refs/tags/v5.7', url_with_token=True)
...
print('══ 4. RELEASE v5.7 ══')
notes = io.open(os.path.join(ROOT, 'docs/RELEASE_NOTES_v5.7.md'), encoding='utf-8').read()
body = {'tag_name': 'v5.7', 'name': '🛡️ ΑΣΠΙΔΑ v5.7 · build VV · 2026-09-22 — README και εισαγωγή στα αγγλικά, για όλο τον κόσμο',
...
if st in (200, 201):
    ...
    mine = [r for r in (rels if isinstance(rels, list) else []) if r.get('tag_name') == 'v5.7']
    if mine:

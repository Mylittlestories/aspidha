...
# -*- coding: utf-8 -*-
"""
PUSH_v5.4.py — δημοσίευση της ΑΣΠΙΔΑΣ v5.4 στο GitHub.
...
ΤΙ ΚΑΝΕΙ (με τη σειρά)
  ...
  4. δημιουργία release v5.4 με τις σημειώσεις από docs/RELEASE_NOTES_v5.4.md.
...
print('══ 2. TAG v5.4 ══')
git('push', '--force', 'https://github.com/%s.git' % REPO, 'refs/tags/v5.4', url_with_token=True)
...
notes = io.open(os.path.join(ROOT, 'docs/RELEASE_NOTES_v5.4.md'), encoding='utf-8').read()
body = {'tag_name': 'v5.4', 'name': 'ΑΣΠΙΔΑ v5.4 — φωτεινή διεπαφή, Πύλη Παραδοτέου',
...
if st in (200, 201):
    ...
    mine = [r for r in (rels if isinstance(rels, list) else []) if r.get('tag_name') == 'v5.4']
    if mine:

...
**English:** [read this README in English](README.md) · **Ζωντανή σελίδα:** <https://Mylittlestories.github.io/aspidha/>

**v5.7 · build VV · 2026-09-22** — ένα αρχείο HTML (≈800 KB), φωτεινή διεπαφή, χωρίς σύνδεση, χωρίς εξαρτήσεις, χωρίς εγκατάσταση.

[![Έκδοση](https://img.shields.io/badge/%CE%AD%CE%BA%CE%B4%CE%BF%CF%83%CE%B7-v5.7-d4af37)](https://github.com/Mylittlestories/aspidha/releases)
[![Έλεγχοι](https://img.shields.io/badge/%CE%AD%CE%BB%CE%B5%CE%B3%CF%87%CE%BF%CE%B9-574%2F574%20%E2%9C%93-27ae60)](tools/verify.js)
...
**Το αποθετήριο και η εισαγωγή της εφαρμογής μιλούν πλέον αγγλικά** — ώστε να καταλαβαίνει ο
καθένας τι είναι το εργαλείο και σε τι χρησιμεύει. Το εργαλείο συνεχίζει να γράφει ελληνικά· ό,τι
άλλαξε είναι γύρω του:

- **Το `README.md` είναι τώρα στα αγγλικά** (το κύριο), και το ελληνικό σώζεται ως
  [`README.el.md`](README.el.md) — αυτό το αρχείο — με συνδέσμους μεταξύ τους.
- **Η εισαγωγή της εφαρμογής είναι διγλωσση.** Η περιγραφή της σελίδας και η κοινή χρήση (og/
  Twitter) αρχίζουν στα αγγλικά, και η κεφαλίδα κρατά μία σύντομη αγγλική γραμμή: «A single-file,
  offline studio that forges the prompt which makes any AI model write your novel the way you
  want…». Η διεπαφή και τα prompts μένουν ελληνικά: το εργαλείο είναι γραμμένο για ελληνικά.
- **Ένα ψευδές εύρημα βρέθηκε και κλείστηκε στην πορεία.** Ο δομικός έλεγχος που επαληθεύει ότι
  κάθε `onclick` έχει υλοποίηση διάβασε το νέο `content="Aspidha (…)"` ως χειριστή «Aspidha» και
  δήλωσε τη δομή σπασμένη. Τώρα το `on…=` θέλει κενό μπροστά — 73 πραγματικοί χειριστές, όλοι
  υλοποιημένοι.

- **Έλεγχος-λαχείο έγινε ντετερμινιστικός.** Ο έλεγχος που θέλει ένα άγνωστο όνομα να καταγγέλλεται
  διάλεγε τον «άγνωστό» του από τον ίδιο κατάλογο απ' όπου κληρώνει ονόματα το εργαλείο: όταν η
  κλήρωση του Name Forge τύχαινε να κλειδώσει εκείνο το όνομα, η πύλη **σωστά** σωπούσε (το
  κλειδωμένο είναι μέρος του συμβολαίου) και ο έλεγχος έσκαγε — μία φορά στις επτά. Τώρα το δείγμα
  χρησιμοποιεί όνομα που αποδεδειγμένα δεν υπάρχει στις λίστες, μηδενίζει τις κλειδαριές, και
  κλειδώνει και το άλλο μισό του κανόνα: μόλις ο άγνωστος υιοθετηθεί, η προειδοποίηση σωπαίνει.
...
- **574 έλεγχοι, 0 αποτυχίες, σε 51 ενότητες.** Η σουίτα ανάπτυξης (`rev.js`) ελέγχει τη
...
```bash
node tools/verify.js            # 33 δομικοί έλεγχοι (χωρίς εξαρτήσεις)
```
...
tools/verify.js                δομικός έλεγχος (33 έλεγχοι) — τρέχει στο CI
.github/workflows/verify.yml   CI: έλεγχος σε κάθε push
README.md                      το README στα αγγλικά (κύριο — για όλους)
README.el.md                   το ίδιο README στα ελληνικά (αυτό το αρχείο)
...
- **Το UI και τα κείμενα είναι στα ελληνικά.** Τα ονόματα των πεδίων είναι ελληνικοί όροι· δεν
  υπάρχει αγγλική μετάφραση της διεπαφής. Το README όμως υπάρχει και στα αγγλικά
  ([README.md](README.md)), ώστε να καταλαβαίνει ο καθένας τι κάνει το εργαλείο.

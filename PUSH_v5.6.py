...
# -*- coding: utf-8 -*-
"""
PUSH_v5.6.py — δημοσίευση της ΑΣΠΙΔΑΣ v5.6 (build UU, 571 έλεγχοι) στο GitHub.
...
ΤΙ ΚΑΝΕΙ (με τη σειρά)
  ...
  4. δημιουργία release v5.6 με τις σημειώσεις από docs/RELEASE_NOTES_v5.6.md.
...
ΧΡΗΣΗ
  GH_TOKEN=ghp_... python3 publish/PUSH_v5.5.py
...
if not TOKEN:
    print('✗ Λείπει το token.  Χρήση:  GH_TOKEN=... python3 publish/PUSH_v5.6.py'); sys.exit(2)
...
print('══ 2. TAG v5.6 ══')
git('push', '--force', 'https://github.com/%s.git' % REPO, 'refs/tags/v5.6', url_with_token=True)
...
print('══ 4. RELEASE v5.6 ══')
notes = io.open(os.path.join(ROOT, 'docs/RELEASE_NOTES_v5.6.md'), encoding='utf-8').read()
body = {'tag_name': 'v5.6', 'name': '🛡️ ΑΣΠΙΔΑ v5.6 · build UU · 2026-09-22 — οδηγίες συνέχειας ανά κεφάλαιο μέσα στο εργαλείο',
...
if st in (200, 201):
    ...
    mine = [r for r in (rels if isinstance(rels, list) else []) if r.get('tag_name') == 'v5.6']
    if mine:
